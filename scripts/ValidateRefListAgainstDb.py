...
def main(args):
	...
	ref_df = pd.read_csv(args.ref_list, sep="\t", header=None, dtype=str)
	if ref_df.shape[1] < 2:
		raise ValueError("ref_list must have at least 2 columns: accession and type")
	ref_df.columns = ["accession", "type"] + [f"col{i}" for i in range(2, ref_df.shape[1])]
	ref_df["accession"] = ref_df["accession"].fillna("").astype(str).str.strip()
	ref_df["type"] = ref_df["type"].fillna("").astype(str).str.lower().str.strip()
	required = set(ref_df[ref_df["type"].isin(["master", "reference"])]["accession"].tolist())
