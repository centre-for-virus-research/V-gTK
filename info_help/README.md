...
| [`guide_alignment_and_insertions.md`](guide_alignment_and_insertions.md) | What happens to a region present in a sub-reference but absent from the master, why insertions are lost without a guide alignment, how to supply one, and the backbone the pipeline builds itself (codon-aware, rare insertions filtered) when you don't. |
...
| [`tree_rooting.md`](tree_rooting.md) | How trees are rooted: midpoint everywhere, why HCV genotype 8 was evaluated as an outgroup and rejected, why single-tip and MMseqs-cluster outgroups do not work, and what happens to UShER node labels, where the build roots trees, and the `iqtree_reference_only` tree. |
...
**Evidence is attached per genotype, not per mutation, one reference per row.**
Each catalogue row carries its own `genotype` and one publication or clinical
trial supporting it *in that genotype, for that drug*: `evidence_id`, with
`data_source` saying which, and `linked_evidence_ids` saying which paper reported
which trial. `NS5A:31M` against daclatasvir has one trial in genotype 1a and ten
in 1b; flattening them would attach one genotype's evidence to another's call.
The `publications` and `clinical_trials` tables are each one row per
`evidence_id`, so a plain equality join resolves a reference and cannot double
the evidence a query reports.
